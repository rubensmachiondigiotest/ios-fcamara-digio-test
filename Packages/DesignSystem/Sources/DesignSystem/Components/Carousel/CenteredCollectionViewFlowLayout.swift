...
public extension UICollectionView {
    /// A convenient way to create a UICollectionView and configure it with a CenteredCollectionViewFlowLayout.
    ///
    /// - Parameters:
    ...
    ///   - centeredCollectionViewFlowLayout: The `CenteredCollectionViewFlowLayout`
    ///   for the `UICollectionView` to be configured with.
...
open class CenteredCollectionViewFlowLayout: UICollectionViewFlowLayout {
    ...
    override open func invalidateLayout(with context: UICollectionViewLayoutInvalidationContext) {
        ...
        let lastScroll = lastScrollDirection != scrollDirection
        let lastSize = lastItemSize != itemSize
        if !currentCollectionViewSize.equalTo(lastCollectionViewSize) || lastScroll || lastSize {
            switch scrollDirection {
            case .horizontal:
...
private extension CenteredCollectionViewFlowLayout {
    ...
    func attributesForRect(
        ...
        for attributes in layoutAttributes {
            ...
            let absX = abs(candidateAttr.center.x - proposedCenterOffset)
            let absY = abs(candidateAttr.center.y - proposedCenterOffset)

            switch scrollDirection {
            case .horizontal where abs(attributes.center.x - proposedCenterOffset) < absX:
                candidateAttributes = attributes

            case .vertical where abs(attributes.center.y - proposedCenterOffset) < absY:
                candidateAttributes = attributes
